...
export default function PageMateriel() {
  ...
  const materiels = data?.pages.flatMap((p: { data: unknown[] }) => p.data) ?? [];
        ...
        <div className="relative w-full overflow-hidden" style={{ height: 'clamp(120px, 20vw, 220px)' }}>
          ...
          <div className="absolute bottom-0 left-0 right-0">
            <div className="max-w-6xl mx-auto px-4 pb-4 md:pb-6">
              <h1 className="text-2xl md:text-4xl font-extrabold text-white drop-shadow">Matériel agricole</h1>
              <p className="text-white/70 text-xs md:text-sm">Louez près de chez vous · Commission 5%</p>
            </div>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20">
              <div className="relative w-full max-w-sm mx-auto rounded-xl overflow-hidden mb-5" style={{ aspectRatio: '16/9' }}>
                <Image src="/images/tracteur.png" alt="Tracteur" fill className="object-cover object-[center_60%] opacity-50" sizes="400px" />
              </div>
              <p className="font-semibold text-foreground-3">Aucun matériel disponible</p>
              <p className="text-sm text-muted-fg mt-1">Soyez le premier à publier</p>
            </div>
          )}
        </div>
      </main>
